...
class Uber {
    ...
    // 2101. Detonate the Maximum Bombs (check)
    fun maximumDetonation(bombs: Array<IntArray>): Int {
        ...
        for (i in 0 until bombs.size) {
            ...
            for (j in i + 1 until bombs.size) {
                ...
                for (k in 0..12) {
                    var angle = slice * k
                    var x = bombs[i][0] + bombs[i][2] * Math.cos(angle)
                    var y = bombs[i][1] + bombs[i][2] * Math.sin(angle)
                    print("point: (${x},${y}) ")

                    if ((x >= comparePair.first) || (y >= comparePair.second)) {
...
fun main(){
    ...
    val bombs = arrayOf(intArrayOf(4,4,3), intArrayOf(4,4,3))
    val testClass = Uber()
